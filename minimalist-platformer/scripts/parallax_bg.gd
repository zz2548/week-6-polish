...
func _ready() -> void:
	...
	# ── Apply persisted toggle state from the autoload ──────────────────────
	# This node is recreated on every reload_current_scene(), so we must
	# restore visibility/emitting from VFXPanel ourselves rather than relying
	# on VFXTogglePanel's side-effect calls (which already ran last session).
	visible        = VFXPanel.parallax_bg_enabled
	_rain.emitting = VFXPanel.rain_enabled
	_rain.visible  = VFXPanel.rain_enabled
...
func _add_rain() -> void:
	...
	_rain.amount   = 300
	_rain.lifetime = 1.5
	# emitting/visible set after _add_rain() returns, in _ready()
